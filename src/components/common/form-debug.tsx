...
export function FormDebug({ form, showErrors = false }: Props) {
	...
	return (
		<div className={cn("relative", showErrors && "shadow-sm bg-muted")}>
			<div className="absolute top-1 right-2 flex items-center gap-2 divide-neutral-950" >
				{isSubmitting && (
					<div className="flex items-center gap-1">
						<span className="text-xs font-medium font-mono">Submitting</span>
						<Loader2 className="size-3.5 animate-spin" />
					</div>
				)}
				{isLoading && (
					<div className="flex items-center gap-1">
						<span className="text-xs font-medium font-mono">Loading</span>
						<Loader2 className="size-3.5 animate-spin" />
					</div>
				)}
				{isValid ? (
					<span className="text-xs font-medium font-mono text-success">Valid</span>
				) : (
					<span className="text-xs font-medium font-mono text-destructive">Invalid</span>
				)}
			...
			<pre className={cn("py-2 overflow-auto border px-3 bg-muted shadow-sm text-foreground font-mono rounded-md", (showErrors && errorss.length > 0) && "rounded-b-none shadow-none")}>{JSON.stringify(form.watch(), null, 2)}</pre>
